...
const REG_CONFIG_VECTOR: u16 = 0x14;
const REG_QUEUE_VECTOR: u16 = 0x16;
...
impl Queue {
    ...
    fn new(port: u16, index: u16, virtual_base: u64) -> Option<Self> {
    ...
    fn describe(&self, index: u16, physical: u64, length: u32, flags: u16) {
        ...
        unsafe {
            ...
            core::ptr::write_volatile((descriptor + 14) as *mut u16, 0);
        }
    }

    /// Offer descriptor `index` to the device.
    fn offer(&mut self, index: u16) {
...
fn bring_up(address: pci::Address) -> Option<VirtioNet> {
    ...
    for index in 0..buffers {
        ...
        nic.receive.describe(index, physical, BUFFER_BYTES as u32, DESCRIPTOR_WRITE);
...
pub fn send(frame: &[u8]) -> Result<(), NetError> {
    ...
    without_interrupts(|| {
        ...
        nic.transmit.describe(0, physical, (HEADER_BYTES + frame.len()) as u32, 0);
